...
|MaxVArLim| ASG|setMag||Nominal max output reactive power at controller|M|
...
| | |swRev||Software revision of the physical device.|R|
| | |model||Software revision of the LN.|R|
| | |mRID||mRID of the physical device. UUIDv4 generated by vendor (?).|R|
| | |serNum||Serialnumber of de physical device|R|
...
| | |q||Quality|M|
| | |t||Timestamp|M|
|Proxy|	SPS| stVal | False | Proxy is not allowed. (?)|M|
| | |q||Quality|M|
| | |t||Timestamp|M|
|PwrUp| SPS| stVal | | Power-up detected |R/O?|
| | |q||Quality|M|
| | |t||Timestamp|M|
